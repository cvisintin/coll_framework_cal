...
model.data <- as.data.table(read.delim("data/cal_model_data_traffic_500.csv", header=T, sep=","))  #Read in traffic volume data for road segments
...
write.csv(vol.preds.df, file = "output/cal_tvol_preds_rf_500.csv", row.names=FALSE)
...
dbWriteTable(con, c("gis_california", "cal_nogeom_roads_volpreds_500"), value = vol.preds.dt, row.names=FALSE, overwrite=TRUE)
...
write.csv(speed.preds.df, file = "output/cal_tspd_preds_rf_500.csv", row.names=FALSE)
...
dbWriteTable(con, c("gis_california", "cal_nogeom_roads_speedpreds_500"), value = speed.preds.dt, row.names=FALSE, overwrite=TRUE)
